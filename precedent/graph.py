...
from precedent.fixtures import seed_decisions
from precedent.schemas import ALLOWED_GRAPH_KEYS, FORBIDDEN_GRAPH_SUBSTRINGS, DerivedDecision
...
class MemoryGraph(DecisionGraph):
    def __init__(self) -> None:
        self._lock = Lock()
        self._rows: dict[str, DerivedDecision] = {d.decision_id: d for d in seed_decisions()}
    ...
    def _ensure_seed(self) -> None:
        ...
        if not existing:
            for d in seed_decisions():
                self._col.document(d.decision_id).set(d.model_dump())
        self._seeded = True
