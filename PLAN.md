# Precedent — Build Plan (not implemented yet)
...
| Three entry points | `/precedent`, `/check-precedent`, passive watcher |
...
`SlackRunner` replies to DMs and mentions. That is the opposite of Precedent. Slack Bolt owns events, slash commands, and ephemeral Block Kit. ADK owns the pipeline as a sequential agent, not a chat loop.
...
| `/check-precedent` | Immediate “Checking this thread…” then card or “No prior decision found” | Never silent |
| `/precedent <query>` | Immediate “Searching decision history…” then card or empty state | Never silent |
...
- Slash: `/precedent`, `/check-precedent`
...
Always ACK in 3 seconds. Work happens on Pub/Sub. Commands use `response_url`. Watcher uses `chat.postEphemeral`.
...
- `PRECEDENT_DAILY_BUDGET_USD` default `10`
...
- **Current precedent** — no warning badge. Decision, why, aftermath, permalink, Share / Not the same.
...
- **Share** — posts a short public summary the user can edit before send. Precedent never posts that on its own.
...
A teammate types “Let’s rotate tokens every night.” Precedent stays out of the channel and privately shows the older 401-cascade decision, why it was made, that it is still current, and a permalink. Chatter gets nothing. A reversed decision is visually louder than a current one. Firestore contains no message text. Fixture evals pass. First GCP bill is under $50.
